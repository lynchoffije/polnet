  ...
  if (!tc)
    return message.channel.send(
      new Discord.MessageEmbed()
        .setColor("BLACK")
        .setDescription(
          `<@${message.author.id}> | Lütfen Sorgulama İçin T.C. Kimlik Numarasını Giriniz. Örnek: \`${prefix}tc [TC] \``
        )
    );

  const url = `https://www.site.com/api/tc.php?tc=${encodeURIComponent(tc)}`;
