  ...
  if (!tc)
    return message.channel.send(
      new Discord.MessageEmbed()
        .setColor("BLACK")
        .setDescription(
          `<@${message.author.id}> | Lütfen Sorgulama İçin T.C. Kimlik Numarasını Giriniz. Örnek: \`${prefix}sülale [TC] \``
        )
    );

  const url = `https://www.site.com/api/sulale.php?tc=${encodeURIComponent(tc)}`;
  ...
  request(url, (error, response, body) => {
    ...
    try {
      ...
      const resultMessage = new Discord.MessageEmbed()
        .setTitle("Sülale Sorgu Sonuçları")
        .addField("TC", data["TC"])
